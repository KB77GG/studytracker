...
def _grade_listening_test_answers(payload: dict, answers: dict, section_number: int | None = None) -> dict:
    return _grade_listening_test_answers_shared(payload, answers, section_number=section_number)


def _grade_reading_test_answers(payload: dict, answers: dict, passage_number: int | None = None) -> dict:
    return _grade_reading_test_answers_shared(payload, answers, passage_number=passage_number)
...
def _listening_intensive_catalog(exercises):
    ...
    for key in sorted(grouped):
        ...
        for test_no in sorted(grouped[key]):
            ...
            last_assigned_values = [
                item.get("last_assigned")
                for item in sections
            ...
            tests.append({
                ...
                "task_count": sum(int(item.get("task_count") or 0) for item in sections),
                "done_count": sum(int(item.get("done_count") or 0) for item in sections),
                "avg_accuracy": round(sum(accuracy_values) / len(accuracy_values), 1)
                if accuracy_values
                else None,
    ...
    return {
        ...
        "test_count": sum(book["test_count"] for book in books),
...
def api_listening_jijing_submit(part_id):
    ...
    grade = _grade_listening_jijing_answers(part, answers)
    correct = grade["correct"]
    total = grade["total"]
    accuracy = grade["accuracy"]
    results = grade["results"]
    wrong_numbers = grade["wrong_numbers"]
    try:
