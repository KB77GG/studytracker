...
判分结果直接读交卷时写入的 ``*_results_json``，不重新判分、不改学生数据。
"""
...
from models import MockExam, MockExamSession, User
...
def exam_session_detail(exam_id, session_id):
    ...
    listening_index = review.build_question_index(
        _load_payload("listening", exam.listening_test_id), "listening"
    )
    reading_index = review.build_question_index(
        _load_payload("reading", exam.reading_test_id), "reading"
    )
    ...
    return render_template(
        ...
        writing_tasks=(writing_payload or {}).get("tasks") or [],
