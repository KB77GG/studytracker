...
class ReadingTestSubmission(db.Model):
    """剑桥雅思阅读整套 Test 的自动判分结果。"""

    __tablename__ = "reading_test_submission"
...
class StudentAnswer(db.Model, TimestampMixin):
    ...
    is_uncertain = db.Column(db.Boolean, default=False, nullable=False, index=True)
...
class StudentVocabularyMastery(db.Model, TimestampMixin):
    ...
    # A small marker for the one related-dimension remediation permitted after
    # a failed same-dimension delayed retry. It is scheduling metadata only;
    # the four dimension stages remain the source of truth.
    review_related_dimension = db.Column(db.String(32), nullable=True)
    review_related_due_at = db.Column(db.DateTime, nullable=True)
    review_remediation_date = db.Column(db.Date, nullable=True)
    review_remediation_count = db.Column(db.Integer, nullable=False, default=0)
...
class VocabularyLearningFlow(db.Model, TimestampMixin):
    ...
    related_source_question_ids_json = db.Column(db.Text, nullable=False, default="[]")
    weak_word_ids_json = db.Column(db.Text, nullable=False, default="[]")
    ...
    remediation_wave = db.Column(db.Integer, nullable=False, default=1)
    pending_correction_question_id = db.Column(db.Integer, nullable=True, index=True)
...
class VocabularyLearningQuestion(db.Model, TimestampMixin):
    ...
    correction_attempt_id = db.Column(db.String(96), nullable=True, index=True)
    correction_is_correct = db.Column(db.Boolean, nullable=True)
    correction_answer = db.Column(db.String(200), nullable=True)
    correction_count = db.Column(db.Integer, nullable=False, default=0)
    remediation_kind = db.Column(db.String(24), nullable=True, index=True)
    source_question_id = db.Column(db.Integer, nullable=True, index=True)
    formal_ordinal = db.Column(db.Integer, nullable=True, index=True)
    deferred_to_review = db.Column(db.Boolean, nullable=False, default=False, index=True)
...
class VocabularyReviewItem(db.Model, TimestampMixin):
    ...
    correction_answer = db.Column(db.String(200), nullable=True)
    correction_count = db.Column(db.Integer, nullable=False, default=0)
    correction_exhausted = db.Column(db.Boolean, nullable=False, default=False)
    remediation_kind = db.Column(db.String(24), nullable=True, index=True)
    deferred_to_review = db.Column(db.Boolean, nullable=False, default=False, index=True)
    state_applied = db.Column(db.Boolean, nullable=False, default=False, index=True)
...
class VocabularyReviewAttempt(db.Model, TimestampMixin):
    ...
    attempt_kind = db.Column(db.String(16), nullable=False, default="first")
