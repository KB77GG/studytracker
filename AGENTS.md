...
2. **`docs/CODEX_HANDOFF.md`** — 跨电脑继续开发时的当前进度、验证结果、发布状态和下一步。
   读取后必须用 `git status --short --branch` 和 `git log -5 --oneline` 核对，不能把可能过期的交接记录当成唯一事实来源。
3. 详细架构见 `docs/ARCHITECTURE.md`。

## 跨电脑进度交接

- 开工时先读 `docs/CODEX_HANDOFF.md`，再核对当前分支、HEAD、远端跟踪状态和工作区改动；工作区不干净时不要盲目 `pull`、切分支或覆盖文件。
- 完成一段重要工作或准备换电脑时，滚动更新 `docs/CODEX_HANDOFF.md`：写明已完成事项、当前提交、测试结果、后端部署状态、小程序上传/审核/发布状态、未完成事项和下一步。
- 只保留当前仍有用的状态与少量近期记录，不粘贴聊天流水账；不要写密码、令牌、Cookie、学生隐私或其他敏感信息。
- 交接文件只能同步已经 commit 并 push 的内容。未提交或未跟踪文件应列出路径和用途，但另一台电脑拿不到其内容；需要跨电脑接续的进行中代码应放到任务分支后提交并推送。
