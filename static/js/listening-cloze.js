(function attachListeningCloze(root, factory) {
  const api = factory();
  if (typeof module === 'object' && module.exports) {
    module.exports = api;
  }
  root.ListeningCloze = api;
}(typeof globalThis !== 'undefined' ? globalThis : this, function createListeningCloze() {
  'use strict';

  // This module intentionally contains no DOM or network code. The mini-program
  // copy is verified against the same fixture in tests/test_listening_cloze.js.
  // Persistent error-reason review and spaced repetition need their own data
  // model, so they are deliberately outside this first stability-focused pass.
  ...
  // These are intentionally configurable rather than a claim that every
  // title-cased word is a name. Days/months stay answerable as information.
  const DEFAULT_PROPER_NOUN_WHITELIST = Object.freeze([
    ...DATE_TIME_WORDS,
  ]);
  ...
  function isLikelyUnspelledName(token, tokens, spellingTarget, options) {
    ...
    // Deliberately only classify title-cased non-initial tokens. This catches
    // normal dialogue names without treating ordinary sentence-initial words
    // such as "Restaurants" as people.
  ...
  function canSitNextTo(candidate, selected) {
    return selected.every(other => {
      ...
      // Adjacent targets are only intentional for a clear high-value sequence,
      // e.g. seven thirty, 23 May, or a spelled-out letter run.
  ...
  function selectHiddenWordIndices(text, level = 'standard', options = {}) {
    ...
    // A one-word acknowledgement cannot become a dead-end. The last-resort
    // fallback only activates when every token was excluded by a safety rule.
    ...
    // Candidate shortages degrade gracefully. Preserve a meaningful non-adjacent
    // selection where possible, then fill only the remaining requested slots.
