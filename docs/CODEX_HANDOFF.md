...
## 2026-08-10 词汇 v2 真机音频 / 输入 / 品牌色热修（后端已部署，小程序待发布）
...
- 正确发布 worktree 仍为 `/Users/zhouxin/.codex/worktrees/9478/studytracker`，分支
  ...
  两个词汇服务及对应测试修改。业务提交已推送到发布分支和 `main` 并部署；本节部署后文档更新会作为 docs-only 提交只推送
  发布分支，避免无业务变化再次重启生产。小程序热修仍未上传/提审/发布。
...
- 线上小程序仍是有上述缺陷的 `16.0.74`。下一步由用户从正确 worktree 上传/提审/发布新热修版（建议 `16.0.75`），然后用 iPhone
  同时验证：静音键开启仍有声、首次进入 `analyst` 自动播、按钮可重播、快速切 3 词不串音、中文只显示选项、英文只显示严格键盘、
  全页品牌绿。已建议先暂停学生继续任务 `3331`，但未修改任务数据；未完成真机验收前不要解除本次热修门禁。
