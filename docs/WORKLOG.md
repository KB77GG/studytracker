...
## 2026-08-14 完整功能小程序已发布（用户确认，待真机联合回归）

- 用户于 22:08 CST 明确确认从正确发布目录完成小程序发布，完整包包含精听升级以及电话/邮编逐字符发音相关客户端修复。
  此处只记录用户确认的外部状态；平台版本号、上传/审核/发布时间和客户端实际拉取版本尚未独立读取。
- 后端仍为已验证的业务提交 `6635b320`，本次状态记录不触发后端重部署。下一步让学生完全退出练习和小程序后重新进入，
  联合回归精听、电话号码及邮编；若仍卡在“正在加载”，保留具体词书序号/原词、发生时间和截图供定位。
