...
## 2026-08-12 词汇任务中途被复习打断 / 确认键无响应（本地修复，待发布）
...
- 当前未 commit/push/deploy，未写生产库，也未上传小程序。必须后端部署 + 小程序重新上传/提审/发布才能完整生效；
  用户已明确授权本轮提交、推送和后端部署，并将在正确目录打开后手动上传/发布小程序。发布后应真机覆盖拼写题、非拼写题、断网/超时重试和已开始任务跨到期点，
  并核验 answer POST、生产服务参数及错误日志。
