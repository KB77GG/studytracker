...
- 在独立 worktree `/Users/zhouxin/.codex/worktrees/86d7/studytracker`、`codex/question-type-review-audio@df1463f9` 完成提交后授权复盘、Reading / Listening 解析与依据、Listening 训练题组范围、逐题有界播放、单音频互斥、当前媒体版本绑定和完整 Section 回退；旧 / 新任务均运行时派生，不改冻结 snapshot/hash/成绩，提交前与考试节奏不泄露复盘数据。
- 首轮独立审阅发现冻结题目与当前 sidecar 不同轴导致提示语错配，以及失效时间轴原文仍可 seek；已改为“冻结坐标选冻结证据行 → 文本语义对齐当前 sidecar”，并统一 transcript / 片段的媒体版本闸门。IELTS21 T3 Q1/Q2 已落到 `Northern Ferries` / `seven days a week`，IELTS20 T1S1 Q6 只落 `Baxter Bridge`；失效合约下原文不可点且只回退完整 Section。
- 第二轮独立复审确认上述两项、fallback 与 Reading 均通过，但抓到 IELTS20 T1S1 Q2 一条冻结长句被 sidecar 拆成两行后漏掉 `roof` 前半句。现一对多映射按词序列选择有新增覆盖的有序行，最终要求冻结整句 100% 覆盖；最终合成 probe 又封住长铺垫提前达标漏短答案，以及多条冻结 evidence 只映射部分的漏洞，缺任一词或整行都回退。Q2 当前 evidence 同时包含 `roof` 与 `need to book`，答案区间 66.84–81.48。
- 全库 84 文件 / 336 Section 时间轴全可用，3,358 / 3,360 题、656 / 658 题组可定位；逐题审计要求每条冻结重叠 evidence 在当前组合 evidence 中 100% 覆盖，未设样例白名单，两道真实历史异常题明确回退。浏览器验证 IELTS20 Q2 两条依据及真实重建轨 83.70 秒自动停；先前语义片段、失效回退、Davies 阅读依据和 console 均已独立复审通过。匿名证据在 `docs/evidence/question-type-review-audio-2026-09-17/`。
- 全仓 Python **756 passed / 72 subtests passed**，最终相关 Python **66 passed / 19 subtests passed**、Node **92 passed**，目标 Ruff / py_compile / JS syntax / diff check 全通过。详细根因、时间基准、命令、统计与未验证项见 `docs/QUESTION_TYPE_REVIEW_AUDIO_EXECUTION.md`。
- 独立终审另跑 Python **54 passed / 2 subtests passed**、Node **55 passed**，两个自建边界 probe、336 Section / 3,358 可用题的 100% 冻结词序覆盖、snapshot 不变、336 个真实 MP3 `ffprobe` 和逐题 / 题组实际时长边界全部通过。Chrome `roof` 64.160604→83.682981 秒自动停，duration 279.84，两条依据完整，console 空；结论 **GO**。
- 当前全部仅本机未提交 / 未跟踪，未 commit / push / deploy，未写生产或学生数据；受版本控制内容没有 MP3、题库、schema、生产配置或小程序改动。5091 合成服务与本轮四个临时媒体 symlink 已全部清理，桌面真实 `ielts20_test1_s1.mp3` 保持普通文件且哈希 `ec0bc0dc…dfa4`。Safari / 微信 WebView / 真机 / 主观听辨未验，下一步等待用户明确授权是否 commit / push / deploy。
...
- 业务 `ea56d107` 已推送任务分支及 main，CI `33933078048`/`33933078074`、Deploy `33933078068` 全部 success。生产 HEAD ea56d107、tracked 干净，service 自 08:28:51 CST active，5002 / 1 worker / 6 threads / NRestarts=0；已部署日期模块边界断言通过，HTTP 302/200/401 符合预期，DB quick_check=ok、外键错误0、warning journal 空。
- 用户明确次日03:00前均可新开始和提交，不要求午夜前开始；具体日期只是举例。固定 release 目录首页哈希与业务提交一致，其他本机候选保留，小程序本轮未上传/提审/发布。最终交接以 skip-ci 文档提交同步；下一步用户上传体验版后真机验收。

## 2026-09-05 — 次日 03:00 截止本地候选完成，等待发布授权

- 在干净分支 `/Users/zhouxin/.codex/worktrees/task-cutoff-3am/studytracker`（`codex/task-cutoff-3am@ac863d36`）把任务可操作窗改为归属日 00:00 至次日 03:00；跨午夜仍归原日期并保留真实提交时间，03:00 起只读。词汇自主复习与计时共用同一闸门，首答/后续作答审计、staff 例外、申诉/报错不变；三条任务通知同步改为次日 03:00。
- 固定上传候选 `/Users/zhouxin/Desktop/studytracker-release/miniprogram` 只叠加首页三文件和一份 Node 回归：首页常驻截止提示，00:00–03:00 可点击进入昨日未完成任务。候选 Node 58/58、no-undef、JS/JSON、官方 WXML/WXSS 均通过；开发者工具实际 projectpath 正确，普通编译 0 errors / 0 warnings，未点上传。
- 后端组合 228 passed，补通知后目标 109 passed；最终全仓为 720 passed、仅 2 个长期缺 MP3 404。当前所有内容仅本机可见，未 commit/push/deploy，未写生产数据，未上传/提审/发布。下一步必须先部署后端，再从固定 release 目录上传并做 00:00–03:00/03:00 边界真机验收。
...
- 登录态实测 `/tasks` 刷新约 2499ms，而材料库 / 词汇审核约 586 / 652ms；问题集中在任务页。周视图虽只展示 10 行，实际把 383 条任务、约 4.34M characters HTML、23,242 个 DOM 节点一次性送到浏览器，Nginx 压缩后仍约 842KB。
- 任务页还会在每次 GET 同步解析 337 份听力 JSON，并内嵌 15,765 个句段；该 JSON 单项约 1.93M characters。服务器当前 active、无重启 / timeout / OOM，数据库 quick_check=ok；同步刷新 CPU 短时峰值约 39% 但未饱和，因此主因是“全量任务 DOM + 全量句段内嵌”，低内存无 Swap 只是额外抖动风险。
- 本轮仅只读诊断，未改代码 / 数据 / 服务，未 commit / push / deploy。建议下一步做服务端分页、资源 / 句段按需 API 加载、资源目录缓存与 N+1 清理，再以生产前后体积和耗时对照验收。

## 2026-09-04 写作练习接入统一任务布置（仅本机）

- 在 `/Users/zhouxin/Desktop/studytracker-writing-mother-topics`、`codex/writing-task-assignment@92bc4441` 把写作接入 `/tasks`：助教 / 教师可搜索并选择 40 道具体真题或 27 个大作文母题，自动带出标题、说明、默认用时，并沿用重复检查、昨日复用、批量发布和 PlanItem 绑定。新增 staff-only 写作目录 API，提交时由服务端校验资源并冻结快照。
- 学生网页今日任务与练习 feed 均获得直达链接；真题完成打字后自动回写任务，母题页可确认完成学习。完成接口校验登录学生、资源、日期和任务归属；无 schema / 题库 / 小程序改动。任务选择器按 Sage Path 既有视觉补充搜索、空态、选中反馈、44px 点击区、焦点和响应式布局。
- 定向 Python **51 passed / 4 subtests**，Node **6 passed**，Python / JS 语法、渲染后 `/tasks` 内联脚本和 diff check 通过；本地 Chrome 验证搜索、筛选、选择和临时内存库发布，临时服务已关闭。当前全部改动未 commit / push / deploy，生产仍为写作母题业务版本 `6eafb2ad`，未写生产或学生数据；小程序未改、未上传 / 提审 / 发布。下一步待用户授权后提交、推送、部署，并用真实助教 / 学生网页账号做端到端验收。

## 2026-09-04 IELTS 大作文母题库已上线

- 在 `/Users/zhouxin/Desktop/studytracker-writing-mother-topics`、`codex/writing-mother-topics` 完成 27 个 Task 2 母题目录与详情，覆盖 252 道用户提供题干；每题含代表题三档范文、4 条五步逻辑链、8 组表达、误区 / 审题 / 迁移训练。现有 30 道大作文与母题双向回链，10 道小作文和打字记录链路不变。
- 延续 Sage Path 现有工作区和品牌色完成目录 / 详情 / 响应式 UI；整合最新主线后本地写作 / 工作区回归 **14 passed / 8 subtests**，JSON、Python / JS 语法和 diff check 通过，桌面与窄屏预览已检查。
- 业务提交 `6eafb2ad` 已原子推送任务分支与 `main`；CI `33824034205`、部署 `33824034233` 均 success。生产 HEAD `6eafb2ad`、tracked 干净，服务 active、`NRestarts=0`、5002 / 1 worker / gthread / 6 threads，SQLite `quick_check=ok` / 外键 0，部署日志无应用错误。公网四个写作入口均 200，27 卡 / 三档范文 / 4 逻辑链 / 28 道 T01 题干 / NO.018 双向链接和打字入口均已核对，静态资产哈希一致。
- 无数据库 / schema / 小程序 / 生产业务数据改动；小程序未上传、提审或发布。本条以 `[skip ci]` 文档提交同步，生产无需二次部署。下一步由用户直接体验并指出需继续细化的母题内容。

## 2026-09-04 学生任务所属日期严格开放闸门（后端已上线，小程序待上传）

- 在独立工作树 `/Users/zhouxin/.codex/worktrees/task-date-gate/studytracker` 的 `codex/task-date-gate` 完成共享 Asia/Shanghai 日期闸门；业务提交 `5c0521ff` 已原子推送任务分支与 `main`。当天可写、未来“尚未开放”、真正未提交的历史任务“未完成·已截止”、已提交待批改任务保持“已提交，待批改”且历史只读、历史完成保留“已完成”但只读；统一 403 机器码/中文消息，教师侧不受影响。同日仍保留既有撤回/继续流程。任务首页、详情和复习中心均保留提交态标签；API 的 expired availability 字段仅表达“已截止”这一日期事实，所有用户可见展示改为优先 workflow status。历史人工答案申诉和题库内容报错不受日期闸门限制，原权限校验保留。
- 接入网页与小程序的任务列表/详情、普通网页听力/阅读旧链接、计时、答案、保存、提交、完成、上传、录音和专项服务入口；读接口不再为未来/过期任务创建新队列、token 或进度，普通网页旧链接也会得到服务端只读上下文并锁定重置/提交。历史完成任务从小程序首页进入通用详情查看成绩/反馈，未来和过期任务统一进入只读详情。新增状态来源与北京午夜边界回归；三条计时链路都允许仅对本人已打开的过期 session 收尾到所属日午夜，23:59:50→00:00:05 计 10 秒且重复 stop 幂等；未来/非本人仍拒绝。为保持测试语义，旧固定日期服务测试已改为日期感知，跨日继续写/结算断言改为 `task_expired`。
- 验证：全量 Python **687 passed / 2 failed / 72 subtests**，两项失败均为当前独立工作树缺少既有 `ielts10_test1_s1.mp3` 夹具（桌面主工作树的未跟踪音频未复制）；发布前最终聚焦集合（日期闸门/计时/展示契约/申诉回归及相关旧测试）**200 passed / 3 subtests**；rollout cutoff 未放宽，历史 strict task 403；Python 编译、JS 语法和 `git diff --check` 通过。
- GitHub CI `33817598499`、部署 `33817598491` 均 success。生产业务 HEAD `5c0521ff`、tracked 干净，service active/running、`NRestarts=0`，5002 / 1 worker / gthread / 6 threads 正常；回环根路由 302，SQLite `quick_check=ok`、外键 0，部署后应用错误计数 0。未写生产数据库或学生业务数据。
- 微信开发者工具已打开并普通编译正确目录 `/Users/zhouxin/.codex/worktrees/task-date-gate/studytracker/miniprogram`；未点击上传，故小程序未上传、提审或发布。工具自动改写的两个项目配置差异已撤回。下一步由用户在当前窗口手动上传/提审/发布，并在真机复核主要任务类型和跨日只读体验。
...
- 验证：Node 全量 **97 passed**；使用 `/Users/zhouxin/Desktop/studytracker/.venv/bin/python` 独立复跑相关 Python `unittest` **21 passed**；`git diff --check` 通过。独立旧版↔候选审计覆盖 195 个 form group / 1,476 个控件；70 个变化表单均归入共享题干、精确前缀去重、label 纠正、静态/context excess 去重或同源 label 单次输出，未解释变化 0，旧版可见非答题源文本丢失 0。旧 9 个 standalone 重复均消失；第三类 source-vs-rendered excess audit 为 0；两个 synthetic 反例、四个指定 shared stem 样例、repeated-context、三处 trailing-value 回归、JFDR6 decorated-label 回归、IELTS 6 `Location:` 相邻 shared cluster 回归、38 个 source-label 保留扫描、全库不变量和普通字段与 shared cluster 之间无数组分隔符回归也通过。初版及中间版的全组文本过滤、context 清空、尾随值丢失、decorated label 错归属与 shared label 缺失均被审阅拦截；当前实现按精确 cell-index 处理重复、共享题干和 direct/group label。
- 业务提交 `7ab2c510` 已原子推送任务分支与 `main`；CI `33646017816`、部署 `33646017805` 均 success。生产 HEAD `7ab2c510`、tracked 干净、服务 active/running、`NRestarts=0`，5002 / 1 worker / gthread / 6 threads 正常；SQLite `quick_check=ok`、外键 0，部署日志无应用错误。公网脚本与本机 / 服务器哈希一致；Chrome 已验证 IELTS 18 T1 S1 尾部重复为 0，以及四个指定 shared stem、10/10 输入框、题号 anchor 和完整性样例全部正确。本轮未写生产业务 / 学生数据，无 schema / 题库 JSON / 小程序改动；小程序未上传、提审或发布。下一步只需学生刷新旧标签页复核。
...
- 现已把所有题型的显式时间戳、错题摘要、底部题号、当前题解析、题面和复盘卡接入同一题目 ID/题号 → Section/start 定位链；首次复盘加载不自动播放，缺失 start 不误跳 0，文本选中和控件操作不误触。84 份 Listening JSON 的 3,360 题均有有效 start。
- Node **84 passed**；定向 Python **23 passed / 8 subtests**；全仓 Python **660 passed / 72 subtests / 2 个既有缺 MP3 夹具失败**；diff check 通过。本地真实浏览器验证 Q7 `≈240.8s`、Q3 `≈127.75s`、跨 Section Q17 `≈349.3s`、题面/复盘卡/显式按钮和 `390×844` 全通过，原文同步、40/40 定位入口与 console `[]` 均通过；临时数据库/音频链接/服务已清理。
- 业务提交 `09a26d60` 已原子推送任务分支与 `main`；CI `33613290896`、部署 `33613290696` 均 success。生产 HEAD `09a26d60`、tracked 干净、服务 active、`NRestarts=0`、5002 / 1 worker / gthread / 6 threads；SQLite `quick_check=ok`、外键 0，部署后无应用错误。公网剑 20 T4S1 已包含 10/10 正确时间入口与新点击链，console `[]`；未在生产提交答案或写业务/学生数据。无 schema/题库/小程序改动，小程序未发布；下一步请学生刷新旧标签后复核实际错题。

## 2026-09-02 网页刷题移动端高亮 / 复盘保留修复已上线
...
- 业务提交 `6335485b` 已原子推送任务分支与 `main`；CI `33609634597`、部署 `33609634380` 均 success。生产 HEAD `6335485b`、tracked 干净、服务 active、`NRestarts=0`，5002 / 1 worker / gthread / 6 threads，部署后无应用错误；SQLite `quick_check=ok`、外键 0。公网高亮脚本哈希与本机 / 服务器一致，四类公开 IELTS 刷题样例均 200 且带高亮根与脚本。
- 准确覆盖为 Listening / Reading 整套、听力 / Reading 机经、复用模板的雅思模考 Listening / Reading，以及题型专项；TOEFL、精听播放器、写作、入学测试和合并模考总复盘从未接入该高亮器，不可宣称全站任意页均支持。生产浏览器交互因控制连接超时未完成，真 iPhone Safari / 微信原生长按仍须学生刷新后验证。本轮无 schema / 数据 / 小程序改动或生产业务数据写入；既有 `artifacts/` 仍未跟踪。
